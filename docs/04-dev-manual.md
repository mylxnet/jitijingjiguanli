...
| 项 | 内容 |
|---|---|
| 文档版本 | v1.3（**§8.3/§10 已随 v0.23.1 更新：前端类型错误清零、`npm run build` 恢复可用**；上一版 v1.2 对齐 v0.23.0 的认证与部署链路，2026-09-20 核对；§1.3 里程碑摘要之外的历史版本明细以 `git log` 为准） |
...
| v0.23.1 | 前端类型错误清零（8 处），修复汇总页下钻跳流水的 `ReferenceError`（点一条流水即报错、跳转失败）；`npm run build` 恢复可用 | ✅ |
| v0.24.0 | **投资收益两类口径改造**（新增预置一级「再投资收益」、532 基数改按两类已收合计、再投资容器只承载本金、迁移 020 回填）、看板银行流水加摘要列、可支出构成负数与环形图渲染修复 | ✅ |
| v0.24.1 | 内嵌静态资源口径修复（缺失 `/assets/*` → 404、`index.html` no-cache、哈希产物 immutable）解决换镜像后"点 tab 没反应"；手机端合同 PDF 改 pdf.js 逐页 canvas + blob 下载兜底 | ✅（真机复核待办） |
| — | PWA 收尾 | ⬜ 待办 |
...
**预置科目（注册即生成 18 项 = 9 个一级 + 9 个二级，preset=1；禁删禁改名，可停用/改期初）**：

| 一级（按 sort 排序） | 预设二级 | 说明 |
|---|---|---|
| 本金 | 上级补助 / 待投资 | 拨款入账与待投池子 |
| 长期投资 | —（建投资类单位时自动建同名二级） | 长期投资本金容器 |
| 再投资 | —（建再投资类单位时自动建同名二级） | 再投资本金容器，**只放本金** |
| 经营收入 | 其他财政收入 / 其他收入 | — |
| 投资收益 | —（收缴长期投资收益时自动建同名二级） | 长期投资收益入账容器 |
| 再投资收益 | —（收缴再投资收益时自动建同名二级） | 再投资收益入账容器（v0.24 起，见 docs/25） |
| 土地流转费收入 | —（建流转类单位时自动建同名二级） | — |
| 流转管理费 | —（同上） | — |
| 分配与支出 | 土地流转费-转付农户 / 成员分红 / 福利发放 / 公益支出 / 管理费支出 | 532 分配支出口径 |

**应收类别 → 现金入账容器（`receivable.recvKindToL1`，唯一控制点）**：

| recvKind | 入账一级容器 |
|---|---|
| `rent` | 土地流转费收入 |
| `service` | 流转管理费 |
| `dividend` | 投资收益（长期投资单位） |
| `reinvest_dividend` | 再投资收益（再投资单位） |

**收益三口径（勿混用，详见 docs/25）**：

| 口径 | 来源 | 用途 |
|---|---|---|
| **已收** | `receipt` 聚合（cash + offset，排除 writeoff 与作废，跨全部年度）→ `/api/parties` 的 `dividendReceivedCents` / `reinvestReceivedCents` | 投资页「已收」卡与列、**532 可分配基数**（= 已收长期 + 已收再投资） |
| 入账现金 | 投资收益 / 再投资收益 两个容器的科目余额 | 仅对账。直接入账未挂应收的收益只出现在这里，页面顶部提示差额，不计入可分配 |
| 年预期收益 | 单位 `expectedReturnCents` | 计提/预览的应计口径，与已收无关 |
...
| 构成卡 | 分项与取数口径 |
|---|---|
| 资金构成 | 银行存款 + 全部在外投资 + 应收收益（`dividend`+`reinvest_dividend` 欠款合计） |
| 在外投资构成 | 投资容器一级（长期投资 / 再投资 / 对外投资）下二级余额，**取绝对值**（负余额视作已投入本金） |
| 欠款构成 | 按 `recv_kind` 分组的 open 应收未收：土地流转费 / 流转管理费 / 应收收益 / 其他 |
| 可支出构成 | 流转管理费一级下二级余额合计 +（**历年累计** 532 `welfare_cents` − **历年累计** 公益支出科目已出）——两侧必须同区间，v0.24.0 前分配额只取最近年度，跨年发放会算成负数 |

> 渲染约定：分项为 0 时不进图例；**负分项不画扇区**（扇区分母只取正分项之和，否则 `stroke-dasharray` 出负值被 SVG 判非法、整环画错），但如实计入合计与图例。中心合计 1 万元以内显示准确数，≥1 万用「x.x万」并带 `title` 精确值。
...
- **532**：可分配基数 = **已收长期投资收益 + 已收再投资收益**（receipt 聚合，见 §2.1 三口径表）；仅按比例记对应账（再投资走资金划转、成员/公益走支出），无方案台账。
- **预置科目**（注册生成 9 个一级 + 9 个二级，明细见 §2.1 预置科目表）：本金 / 长期投资 / 再投资 / 经营收入 / 投资收益 / 再投资收益 / 土地流转费收入 / 流转管理费 / 分配与支出。前三个投资容器与两个收益容器分工——本金容器只放本金，收益入「投资收益」「再投资收益」。
...
> 依赖预置科目名定位；若科目被改名会提示“缺少科目：XX”。「收到投资收益/分红」按所选单位类型选容器：再投资单位 → 「再投资收益」，其余 → 「投资收益」（与后端 `recvKindToL1` 同口径）；该单位有对应类别欠单时走收缴核销，无欠单才直记收入科目。
...
cd web && npm run build                      # = vue-tsc -b && vite build，类型检查 + 构建一起过（v0.23.1 起可用）
cd web && npx vue-tsc --noEmit               # 只跑类型检查；应为 0 报错
```

✅ 自 **v0.23.1** 起，历史遗留的 8 处类型错误已全部修复，`npm run build` **恢复可用**，本地验收优先用它（能把类型问题一起拦住）。
`.github/workflows/release.yml` 仍写 `npx vite build`（跳过类型检查）—— 现可收紧为 `npm run build`，属遗留优化项，见 §10。
...
| 前端类型检查 | **已清零**（v0.23.1 修掉 8 处，含 `SummaryPage.vue` 汇总页下钻的 `range` 未定义运行时 bug）；历史清单与验收记录保留在 `docs/22-前端待修类型错误清单.md` |
| CI 构建口径 | `.github/workflows/release.yml` 仍用 `npx vite build`（跳过 vue-tsc）。本地已恢复 `npm run build`，CI 可在下轮收紧 |
...
| 看板「银行存款流水」弹窗 | 未修（v0.24.0 只加了摘要列）：取数是**全部收支流水**、缺资金划转（对外投资/收回）行，而划转确实改变银行存款余额 → 表末「余额」与首页银行存款卡对不上；且未过滤 `status='voided'` 的作废流水 |
| 投资页签金额取数 | 未修：长期/再投资页签仍按**同名挂接**取科目余额，且对负本金取 `Math.abs`（例：某单位期初 420,000、投出 550,000 → 余额 −130,000 被洗成 +¥1,300 计入再投资总额）。根治需给 `category` 加 `party_id` |
| 快速记账「收到投资收益/分红」 | 未修：所选单位**无未结清同类应收单**时静默回落成普通收入记账（只落收益科目、不产生 `receipt`，因而不进「已收」与可分配池），界面无任何区分提示；只能事后靠投资收益页签的对账黄条发现 |
| 532 分配基数不按年度归集 | 现状为**滚动池**：已收与已分配都是历年累计，`未分配 = max(0, 累计已收 − 历年已分配)`，收款日期不参与年度归属。若要权责发生制（当年收益当年分、超期单列结转）需另开口径 |
| PWA 收尾、启动自检（网络盘检测） | 设计有、未实现 |
| 内嵌前端静态资源口径 | **已修（v0.24.1）**：`/assets/*` 未命中 404、`index.html` `no-cache`、哈希产物 `immutable`（测试 `server/cmd/server/main_test.go`）；**「版本漂移自愈」仍未做**，旧标签页仍需刷新一次。详见 `docs/26-静态资源缓存与手机端合同PDF预览方案.md` |
| 手机端合同 PDF 预览 | **已改（v0.24.1）**：pdf.js 逐页画 canvas + blob 下载兜底。**真机（安卓/微信）未实机复核**，非 PDF 类型的 blob 链路亦未实测 |
...
| 2026-09-20 | v1.3：随 v0.23.1 —— §1.3 增补补丁版行；§8.3 质量关卡改为以 `npm run build` 为准（8 处类型错误已清零）；§10 把「前端类型检查欠账」改为已修复，并新增「CI 仍走 vite build 可收紧」遗留项 |
| 2026-09-21 | v1.4：随 v0.24.0 —— §1.3 增补版本行；§2.1 预置科目表重写为 9 一级 / 9 二级（18 项）并新增「应收类别 → 现金入账容器」与「收益三口径」两张表；§2.4 新增看板环形构成取数与渲染约定表；§2.8 更正两条过时条目；§7.7 补「收到投资收益/分红按单位类型选容器」；§10 登记「银行存款流水缺划转行与未过滤 voided」「负本金被取绝对值」「无欠单时收缴静默回落成收入」三条已知未修 |
| 2026-09-21 | v1.5：随 v0.24.1 —— §10 登记内嵌静态资源口径（`/assets/*` 404 + `index.html` no-cache + 哈希产物 immutable，测试 `server/cmd/server/main_test.go`）与手机端合同 PDF 预览改造（pdf.js canvas + blob 下载兜底，真机未复核）；方案与取证见 `docs/26-静态资源缓存与手机端合同PDF预览方案.md` |
